...
% upstream_sha256: d32f4ab303070c31733cd624979a0b5ff3cb43f8d652f0610f7a4c6d249e186e
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[height=3.38cm]{figures/probability_models/dices}
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[height=4.125cm]{figures/probability_models/nonadmissiblespace}
...
\begin{figure}[htb!]
\begin{center}
\includegraphics[height=3.75cm]{figures/probability_models/admissiblespace}
\end{center}
...
In other areas of mathematics, the same object is known as a \emph{probability measure}, a name that emphasizes its formal status as a set function defined on an admissible class of events;\index{Probability measure} we return to this connection in the starred section at the end of the chapter.
\begin{enumerate}
...
For instance, an event $A$ and its complement $A^{\Complement}$ are disjoint and together exhaust the sample space, so nonnegativity, normalization, and additivity yield the \emph{complement rule}\index{Complement rule}
\begin{equation*}
\Pr (A^{\Complement}) = 1 - \Pr (A) .
\end{equation*}
Taking $A = \Omega$ shows in particular that the impossible event carries no probability, $\Pr (\emptyset) = 0$.
We prove two further properties below.
...
The monotonicity property has a simple corollary that human intuition routinely violates.
Because the joint event $A \cap B$ is contained in $A$, it follows that $\Pr (A \cap B) \leq \Pr (A)$;
a conjunction of two conditions can never be more probable than either condition on its own.
The tendency to judge a detailed and representative scenario as more likely than one of its own constituents is a documented reasoning error known as the \emph{conjunction fallacy}.\index{Conjunction fallacy}
In a classic demonstration, respondents who are told that a person is politically active are asked whether that person is more likely to be a bank teller, or a bank teller who is also involved in a social movement;
many rank the more specific, and therefore less probable, description higher.
...
\end{split}
\end{equation*}
Then, by the principle of mathematical induction, \eqref{equation:UnionBound} is valid for all positive integers.
\end{proof}
...
The equally likely model is a modeling assumption, not a logical necessity, and it must be justified for each experiment rather than presumed.
The unexamined belief that the outcomes of any experiment are automatically equally probable is a common misconception, sometimes called the \emph{outcome approach}.\index{Outcome approach}
A loaded die, a biased coin, and the sum of two dice all have outcomes that are not equally likely;
the integers two through twelve, for instance, are far from equiprobable as totals of two fair dice.
Whenever the equally likely model is invoked, it should be traceable to a genuine symmetry of the underlying experiment.
...
\end{equation*}
as expected.
Similarly, the probability of the number of coin tosses being even is given by
\begin{equation*}
\Pr ( \{ 2, 4, 6, \ldots \} )
...
The axioms of probability have a further consequence that becomes indispensable when working with sequences of events.
Consider an increasing sequence of events $A_1 \subset A_2 \subset \cdots$, and let $A = \bigcup_{k=1}^{\infty} A_k$ denote its limit.
It is natural to expect the probability of $A_n$ to approach the probability of $A$ as $n$ grows without bound.
This is indeed the case.

\begin{proposition}[Continuity from Below]
If $A_1 \subset A_2 \subset \cdots$ is an increasing sequence of events with union $A = \bigcup_{k=1}^{\infty} A_k$, then
\begin{equation*}
\Pr (A) = \lim_{n \to \infty} \Pr (A_n) .
\end{equation*}
\end{proposition}
\begin{proof}
Define $B_1 = A_1$ and $B_k = A_k - A_{k-1}$ for $k \geq 2$.
The events $B_1, B_2, \ldots$ are disjoint, with $\bigcup_{k=1}^{n} B_k = A_n$ for every $n$ and $\bigcup_{k=1}^{\infty} B_k = A$.
Applying the third axiom of probability, we get
\begin{equation*}
\Pr (A)
= \sum_{k=1}^{\infty} \Pr (B_k)
= \lim_{n \to \infty} \sum_{k=1}^{n} \Pr (B_k)
= \lim_{n \to \infty} \Pr (A_n) ,
\end{equation*}
which is the desired result.
\end{proof}

A symmetric statement holds for a decreasing sequence of events $A_1 \supset A_2 \supset \cdots$.
Writing $A = \bigcap_{k=1}^{\infty} A_k$, one obtains $\Pr (A) = \lim_{n \to \infty} \Pr (A_n)$ by applying continuity from below to the complements $A_k^{\Complement}$ and invoking the complement rule.
Together, these two results are known as the \emph{continuity of probability}.\index{Continuity of probability}

...
% upstream_sha256: 94d88b8fbede918561e19adc4ffc59ec91443607682502db9c4c5dd40987d876
...
Below, we offer a description of continuous random variables.
...
\begin{equation*}
\begin{split}
p_X (k) &= F_X (k) - \lim_{u \uparrow k} F_X (u) = F_X (k) - F_X (k-1) \\
...
Having introduced the general notion of a CDF, we turn our attention to characteristics of continuous random variables.
Let $X$ be a random variable with CDF $F_X (\cdot)$.
Furthermore, supposed that $F_X (\cdot)$ is a continuous function, and that it is differentiable almost everywhere (except perhaps at discrete locations).
Then, $X$ is said to be a \emph{continuous random variable}. \index{Continuous random variable}

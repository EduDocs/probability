...
% upstream_sha256: ab5eb79527c0d9637efdcfa6784f030f5182491fe6943da79f73909398fd53e5
...
S = \{ x_1, x_2, x_3 \} .
\end{equation*}
The content of a set can also be specified by an unending list, provided its elements can be arranged in a sequence,
\begin{equation*}
S = \{ x_1, x_2, \ldots \} .
\end{equation*}
...
Once a universal set $\Omega$ is fixed, we need only consider sets that are subsets of $\Omega$.
...
A collection of nonempty sets is said to form a \emph{partition} of $S$ if the sets in the collection are disjoint and their union is $S$.\index{Partition}
...
Above, $S$ is partitioned into three disjoint subsets: $S_1$, $S_2$, and $S_3$.}
\end{center}
\end{figure}
...
\end{align*}
where the index set $\IndexSet$ can be finite or infinite.
...
The Cartesian product gives us the language to describe correspondences between sets.
A \emph{relation} between sets $X$ and $Y$ is any subset of the Cartesian product $X \times Y$;\index{Relation}
when the pair $(x, y)$ belongs to the relation, we say that $x$ is related to $y$.
A \emph{function} is a relation with a special property: it associates with every element of $X$ exactly one element of $Y$.\index{Function}
That is, $f \subset X \times Y$ is a function if, for each $x \in X$, there is one and only one $y \in Y$ for which $(x, y) \in f$.
This unique value is written $f(x)$, so that $f(x) = y$ exactly when $(x, y) \in f$, and the rule that produces $f(x)$ from $x$ is called the rule of correspondence.
The \emph{domain} of a function is the set over which it is defined; that is, the collection of arguments that can be used as input.\index{Domain}
...
Formally, however, a function is precisely a triple $(X, Y, f)$ in which the \emph{graph} $f \subset X \times Y$ obeys the single-value property stated above.
...
Suppose $S$ is a subset of the universal set $\Omega$.
We define the indicator function of set $S$, denoted $\IndicatorFcn_S: \Omega \rightarrow \{ 0, 1 \}$, by
\begin{equation*}
...
When $\Omega$ plays the role of a sample space, the indicator of $S$ reports whether an observed outcome falls in $S$; this is the seed of the Bernoulli random variable encountered later.
\end{example}
...
\begin{tikzpicture}
\begin{axis}[
  width=4in, height=2.8in,
  axis lines=middle,
  xlabel={$x$}, ylabel={$f(x)$},
  xlabel style={right}, ylabel style={above},
  xtick={-1.4142,1.4142}, xticklabels={$-\sqrt{c}$,$\sqrt{c}$},
  ytick={2}, yticklabels={$c$}, yticklabel style={yshift=7pt},
  xmin=-2.4, xmax=2.4, ymin=-0.5, ymax=4.4,
  samples=100, clip=false,
]
\addplot[thick, domain=-2.05:2.05] {x^2};
\draw[dashed] (axis cs:-2.4,2) -- (axis cs:2.4,2);
\draw[dashed] (axis cs:-1.4142,2) -- (axis cs:-1.4142,0);
\draw[dashed] (axis cs:1.4142,2) -- (axis cs:1.4142,0);
\node[circle,fill,inner sep=1.5pt] at (axis cs:-1.4142,2) {};
\node[circle,fill,inner sep=1.5pt] at (axis cs:1.4142,2) {};
\node[circle,fill,inner sep=1.5pt] at (axis cs:-1.4142,0) {};
\node[circle,fill,inner sep=1.5pt] at (axis cs:1.4142,0) {};
\end{axis}
\end{tikzpicture}
\caption{The level set of a value $c > 0$ under $f(x) = x^2$ is its preimage $f^{-1} (\{ c \}) = \{ -\sqrt{c}, \sqrt{c} \}$.
The horizontal line at height $c$ meets the graph at two points, whose arguments make up the preimage.
This illustrates that the preimage of a single value may contain several elements.}
\end{center}
\end{figure}
...
Many of the constructs assembled in this chapter reappear in a probabilistic guise.
The universal set becomes the sample space of an experiment, and a partition of it underlies the law of total probability.
The preimage of a function is the mechanism behind random variables, while the Cartesian product lets us describe the joint behavior of several quantities at once.
